...
export default function DashboardBody(props) {
    ...
    useEffect(()=> {
        Axios.get(`http://localhost:8081/products/${props.productId}`)
            .then(res => {
                setProduct(res.data)
            } 
        )
        Axios.get(`http://localhost:8081/news/${props.productId}`)
            .then(res => {
                setNews(res.data)
            } 
        )
        Axios.get(`http://localhost:8081/users/list/${props.productId}`)
            .then(res => {
                setUser(res.data)
            } 
        )
        Axios.get(`http://localhost:8081/order/${props.productId}`)
            .then(res => {
                setOrder(res.data)
            } 
        )
        Axios.get(`http://localhost:8081/collection/${props.productId}`)
            .then(res => {
                setCollection(res.data)
            } 
        )
        Axios.get(`http://localhost:8081/email/${props.productId}`)
            .then(res => {
                setEmail(res.data)
            } 
        )
